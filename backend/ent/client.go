...
type Client struct {
	...
	// AdapterProvider is the client for interacting with the AdapterProvider builders.
	AdapterProvider *AdapterProviderClient
	// AdapterRequest is the client for interacting with the AdapterRequest builders.
	AdapterRequest *AdapterRequestClient
	// AdapterUsageRecord is the client for interacting with the AdapterUsageRecord builders.
	AdapterUsageRecord *AdapterUsageRecordClient
...
func (c *Client) init() {
	...
	c.AdapterProvider = NewAdapterProviderClient(c.config)
	c.AdapterRequest = NewAdapterRequestClient(c.config)
	c.AdapterUsageRecord = NewAdapterUsageRecordClient(c.config)
...
func (c *Client) Tx(ctx context.Context) (*Tx, error) {
	...
	return &Tx{
		...
		AdapterProvider:               NewAdapterProviderClient(cfg),
		AdapterRequest:                NewAdapterRequestClient(cfg),
		AdapterUsageRecord:            NewAdapterUsageRecordClient(cfg),
...
func (c *Client) Use(hooks ...Hook) {
	for _, n := range []interface{ Use(...Hook) }{
		c.APIKey, c.Account, c.AccountGroup, c.AdapterProvider, c.AdapterRequest,
		c.AdapterUsageRecord, c.Announcement, c.AnnouncementRead, c.AuthIdentity,
		c.AuthIdentityChannel, c.ChannelMonitor, c.ChannelMonitorDailyRollup,
		c.ChannelMonitorHistory, c.ChannelMonitorRequestTemplate,
		c.ErrorPassthroughRule, c.Group, c.IdempotencyRecord,
		c.IdentityAdoptionDecision, c.PaymentAuditLog, c.PaymentOrder,
		c.PaymentProviderInstance, c.PendingAuthSession, c.PromoCode, c.PromoCodeUsage,
		c.Proxy, c.RedeemCode, c.RoutePolicy, c.SecuritySecret, c.Setting,
...
func (c *Client) Mutate(ctx context.Context, m Mutation) (Value, error) {
	...
	case *AdapterProviderMutation:
		return c.AdapterProvider.mutate(ctx, m)
	case *AdapterRequestMutation:
		return c.AdapterRequest.mutate(ctx, m)
	case *AdapterUsageRecordMutation:
		return c.AdapterUsageRecord.mutate(ctx, m)
...
// QueryAdapterRequests queries the adapter_requests edge of a APIKey.
func (c *APIKeyClient) QueryAdapterRequests(_m *APIKey) *AdapterRequestQuery {
	query := (&AdapterRequestClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(apikey.Table, apikey.FieldID, id),
			sqlgraph.To(adapterrequest.Table, adapterrequest.FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, apikey.AdapterRequestsTable, apikey.AdapterRequestsColumn),
		)
		fromV = sqlgraph.Neighbors(_m.driver.Dialect(), step)
		return fromV, nil
	}
...
// QueryAdapterUsageRecords queries the adapter_usage_records edge of a APIKey.
func (c *APIKeyClient) QueryAdapterUsageRecords(_m *APIKey) *AdapterUsageRecordQuery {
	query := (&AdapterUsageRecordClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(apikey.Table, apikey.FieldID, id),
			sqlgraph.To(adapterusagerecord.Table, adapterusagerecord.FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, apikey.AdapterUsageRecordsTable, apikey.AdapterUsageRecordsColumn),
		)
		fromV = sqlgraph.Neighbors(_m.driver.Dialect(), step)
		return fromV, nil
	}
...
// AdapterProviderClient is a client for the AdapterProvider schema.
type AdapterProviderClient struct {
	config
}

// NewAdapterProviderClient returns a client for the AdapterProvider from the given config.
func NewAdapterProviderClient(c config) *AdapterProviderClient {
	return &AdapterProviderClient{config: c}
}

// Use adds a list of mutation hooks to the hooks stack.
// A call to `Use(f, g, h)` equals to `adapterprovider.Hooks(f(g(h())))`.
func (c *AdapterProviderClient) Use(hooks ...Hook) {
	c.hooks.AdapterProvider = append(c.hooks.AdapterProvider, hooks...)
}

// Intercept adds a list of query interceptors to the interceptors stack.
// A call to `Intercept(f, g, h)` equals to `adapterprovider.Intercept(f(g(h())))`.
func (c *AdapterProviderClient) Intercept(interceptors ...Interceptor) {
	c.inters.AdapterProvider = append(c.inters.AdapterProvider, interceptors...)
}

// Create returns a builder for creating a AdapterProvider entity.
func (c *AdapterProviderClient) Create() *AdapterProviderCreate {
	mutation := newAdapterProviderMutation(c.config, OpCreate)
	return &AdapterProviderCreate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// CreateBulk returns a builder for creating a bulk of AdapterProvider entities.
func (c *AdapterProviderClient) CreateBulk(builders ...*AdapterProviderCreate) *AdapterProviderCreateBulk {
	return &AdapterProviderCreateBulk{config: c.config, builders: builders}
}

// MapCreateBulk creates a bulk creation builder from the given slice. For each item in the slice, the function creates
// a builder and applies setFunc on it.
func (c *AdapterProviderClient) MapCreateBulk(slice any, setFunc func(*AdapterProviderCreate, int)) *AdapterProviderCreateBulk {
	rv := reflect.ValueOf(slice)
	if rv.Kind() != reflect.Slice {
		return &AdapterProviderCreateBulk{err: fmt.Errorf("calling to AdapterProviderClient.MapCreateBulk with wrong type %T, need slice", slice)}
	}
	builders := make([]*AdapterProviderCreate, rv.Len())
	for i := 0; i < rv.Len(); i++ {
		builders[i] = c.Create()
		setFunc(builders[i], i)
	}
...
// Update returns an update builder for AdapterProvider.
func (c *AdapterProviderClient) Update() *AdapterProviderUpdate {
	mutation := newAdapterProviderMutation(c.config, OpUpdate)
	return &AdapterProviderUpdate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOne returns an update builder for the given entity.
func (c *AdapterProviderClient) UpdateOne(_m *AdapterProvider) *AdapterProviderUpdateOne {
	mutation := newAdapterProviderMutation(c.config, OpUpdateOne, withAdapterProvider(_m))
	return &AdapterProviderUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOneID returns an update builder for the given id.
func (c *AdapterProviderClient) UpdateOneID(id int64) *AdapterProviderUpdateOne {
	mutation := newAdapterProviderMutation(c.config, OpUpdateOne, withAdapterProviderID(id))
	return &AdapterProviderUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// Delete returns a delete builder for AdapterProvider.
func (c *AdapterProviderClient) Delete() *AdapterProviderDelete {
	mutation := newAdapterProviderMutation(c.config, OpDelete)
	return &AdapterProviderDelete{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// DeleteOne returns a builder for deleting the given entity.
func (c *AdapterProviderClient) DeleteOne(_m *AdapterProvider) *AdapterProviderDeleteOne {
	return c.DeleteOneID(_m.ID)
}

// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *AdapterProviderClient) DeleteOneID(id int64) *AdapterProviderDeleteOne {
	builder := c.Delete().Where(adapterprovider.ID(id))
	builder.mutation.id = &id
	builder.mutation.op = OpDeleteOne
	return &AdapterProviderDeleteOne{builder}
}

// Query returns a query builder for AdapterProvider.
func (c *AdapterProviderClient) Query() *AdapterProviderQuery {
	return &AdapterProviderQuery{
		config: c.config,
		ctx:    &QueryContext{Type: TypeAdapterProvider},
		inters: c.Interceptors(),
	}
}

// Get returns a AdapterProvider entity by its id.
func (c *AdapterProviderClient) Get(ctx context.Context, id int64) (*AdapterProvider, error) {
	return c.Query().Where(adapterprovider.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *AdapterProviderClient) GetX(ctx context.Context, id int64) *AdapterProvider {
	obj, err := c.Get(ctx, id)
	if err != nil {
		panic(err)
	}
...
// QueryRoutePolicies queries the route_policies edge of a AdapterProvider.
func (c *AdapterProviderClient) QueryRoutePolicies(_m *AdapterProvider) *RoutePolicyQuery {
	query := (&RoutePolicyClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(adapterprovider.Table, adapterprovider.FieldID, id),
			sqlgraph.To(routepolicy.Table, routepolicy.FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, adapterprovider.RoutePoliciesTable, adapterprovider.RoutePoliciesColumn),
		)
		fromV = sqlgraph.Neighbors(_m.driver.Dialect(), step)
		return fromV, nil
	}
...
// QueryAdapterRequests queries the adapter_requests edge of a AdapterProvider.
func (c *AdapterProviderClient) QueryAdapterRequests(_m *AdapterProvider) *AdapterRequestQuery {
	query := (&AdapterRequestClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(adapterprovider.Table, adapterprovider.FieldID, id),
			sqlgraph.To(adapterrequest.Table, adapterrequest.FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, adapterprovider.AdapterRequestsTable, adapterprovider.AdapterRequestsColumn),
		)
		fromV = sqlgraph.Neighbors(_m.driver.Dialect(), step)
		return fromV, nil
	}
...
// QueryAdapterUsageRecords queries the adapter_usage_records edge of a AdapterProvider.
func (c *AdapterProviderClient) QueryAdapterUsageRecords(_m *AdapterProvider) *AdapterUsageRecordQuery {
	query := (&AdapterUsageRecordClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(adapterprovider.Table, adapterprovider.FieldID, id),
			sqlgraph.To(adapterusagerecord.Table, adapterusagerecord.FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, adapterprovider.AdapterUsageRecordsTable, adapterprovider.AdapterUsageRecordsColumn),
		)
		fromV = sqlgraph.Neighbors(_m.driver.Dialect(), step)
		return fromV, nil
	}
...
// Hooks returns the client hooks.
func (c *AdapterProviderClient) Hooks() []Hook {
	hooks := c.hooks.AdapterProvider
	return append(hooks[:len(hooks):len(hooks)], adapterprovider.Hooks[:]...)
}

// Interceptors returns the client interceptors.
func (c *AdapterProviderClient) Interceptors() []Interceptor {
	inters := c.inters.AdapterProvider
	return append(inters[:len(inters):len(inters)], adapterprovider.Interceptors[:]...)
}

func (c *AdapterProviderClient) mutate(ctx context.Context, m *AdapterProviderMutation) (Value, error) {
	switch m.Op() {
	case OpCreate:
		return (&AdapterProviderCreate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdate:
		return (&AdapterProviderUpdate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdateOne:
		return (&AdapterProviderUpdateOne{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpDelete, OpDeleteOne:
		return (&AdapterProviderDelete{config: c.config, hooks: c.Hooks(), mutation: m}).Exec(ctx)
	default:
		return nil, fmt.Errorf("ent: unknown AdapterProvider mutation op: %q", m.Op())
	}
}

// AdapterRequestClient is a client for the AdapterRequest schema.
type AdapterRequestClient struct {
	config
}

// NewAdapterRequestClient returns a client for the AdapterRequest from the given config.
func NewAdapterRequestClient(c config) *AdapterRequestClient {
	return &AdapterRequestClient{config: c}
}

// Use adds a list of mutation hooks to the hooks stack.
// A call to `Use(f, g, h)` equals to `adapterrequest.Hooks(f(g(h())))`.
func (c *AdapterRequestClient) Use(hooks ...Hook) {
	c.hooks.AdapterRequest = append(c.hooks.AdapterRequest, hooks...)
}

// Intercept adds a list of query interceptors to the interceptors stack.
// A call to `Intercept(f, g, h)` equals to `adapterrequest.Intercept(f(g(h())))`.
func (c *AdapterRequestClient) Intercept(interceptors ...Interceptor) {
	c.inters.AdapterRequest = append(c.inters.AdapterRequest, interceptors...)
}

// Create returns a builder for creating a AdapterRequest entity.
func (c *AdapterRequestClient) Create() *AdapterRequestCreate {
	mutation := newAdapterRequestMutation(c.config, OpCreate)
	return &AdapterRequestCreate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// CreateBulk returns a builder for creating a bulk of AdapterRequest entities.
func (c *AdapterRequestClient) CreateBulk(builders ...*AdapterRequestCreate) *AdapterRequestCreateBulk {
	return &AdapterRequestCreateBulk{config: c.config, builders: builders}
}

// MapCreateBulk creates a bulk creation builder from the given slice. For each item in the slice, the function creates
// a builder and applies setFunc on it.
func (c *AdapterRequestClient) MapCreateBulk(slice any, setFunc func(*AdapterRequestCreate, int)) *AdapterRequestCreateBulk {
	rv := reflect.ValueOf(slice)
	if rv.Kind() != reflect.Slice {
		return &AdapterRequestCreateBulk{err: fmt.Errorf("calling to AdapterRequestClient.MapCreateBulk with wrong type %T, need slice", slice)}
	}
	builders := make([]*AdapterRequestCreate, rv.Len())
	for i := 0; i < rv.Len(); i++ {
		builders[i] = c.Create()
		setFunc(builders[i], i)
	}
...
// Update returns an update builder for AdapterRequest.
func (c *AdapterRequestClient) Update() *AdapterRequestUpdate {
	mutation := newAdapterRequestMutation(c.config, OpUpdate)
	return &AdapterRequestUpdate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOne returns an update builder for the given entity.
func (c *AdapterRequestClient) UpdateOne(_m *AdapterRequest) *AdapterRequestUpdateOne {
	mutation := newAdapterRequestMutation(c.config, OpUpdateOne, withAdapterRequest(_m))
	return &AdapterRequestUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOneID returns an update builder for the given id.
func (c *AdapterRequestClient) UpdateOneID(id int64) *AdapterRequestUpdateOne {
	mutation := newAdapterRequestMutation(c.config, OpUpdateOne, withAdapterRequestID(id))
	return &AdapterRequestUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// Delete returns a delete builder for AdapterRequest.
func (c *AdapterRequestClient) Delete() *AdapterRequestDelete {
	mutation := newAdapterRequestMutation(c.config, OpDelete)
	return &AdapterRequestDelete{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// DeleteOne returns a builder for deleting the given entity.
func (c *AdapterRequestClient) DeleteOne(_m *AdapterRequest) *AdapterRequestDeleteOne {
	return c.DeleteOneID(_m.ID)
}

// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *AdapterRequestClient) DeleteOneID(id int64) *AdapterRequestDeleteOne {
	builder := c.Delete().Where(adapterrequest.ID(id))
	builder.mutation.id = &id
	builder.mutation.op = OpDeleteOne
	return &AdapterRequestDeleteOne{builder}
}

// Query returns a query builder for AdapterRequest.
func (c *AdapterRequestClient) Query() *AdapterRequestQuery {
	return &AdapterRequestQuery{
		config: c.config,
		ctx:    &QueryContext{Type: TypeAdapterRequest},
		inters: c.Interceptors(),
	}
}

// Get returns a AdapterRequest entity by its id.
func (c *AdapterRequestClient) Get(ctx context.Context, id int64) (*AdapterRequest, error) {
	return c.Query().Where(adapterrequest.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *AdapterRequestClient) GetX(ctx context.Context, id int64) *AdapterRequest {
	obj, err := c.Get(ctx, id)
	if err != nil {
		panic(err)
	}
...
// QueryUser queries the user edge of a AdapterRequest.
func (c *AdapterRequestClient) QueryUser(_m *AdapterRequest) *UserQuery {
	query := (&UserClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(adapterrequest.Table, adapterrequest.FieldID, id),
			sqlgraph.To(user.Table, user.FieldID),
...
// QueryAPIKey queries the api_key edge of a AdapterRequest.
func (c *AdapterRequestClient) QueryAPIKey(_m *AdapterRequest) *APIKeyQuery {
...
// QueryGroup queries the group edge of a AdapterRequest.
func (c *AdapterRequestClient) QueryGroup(_m *AdapterRequest) *GroupQuery {
	query := (&GroupClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
...
// QueryAdapterProvider queries the adapter_provider edge of a AdapterRequest.
func (c *AdapterRequestClient) QueryAdapterProvider(_m *AdapterRequest) *AdapterProviderQuery {
	query := (&AdapterProviderClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(adapterrequest.Table, adapterrequest.FieldID, id),
...
// Hooks returns the client hooks.
func (c *AdapterRequestClient) Hooks() []Hook {
	return c.hooks.AdapterRequest
}

// Interceptors returns the client interceptors.
func (c *AdapterRequestClient) Interceptors() []Interceptor {
	return c.inters.AdapterRequest
}

func (c *AdapterRequestClient) mutate(ctx context.Context, m *AdapterRequestMutation) (Value, error) {
	switch m.Op() {
	case OpCreate:
		return (&AdapterRequestCreate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdate:
		return (&AdapterRequestUpdate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdateOne:
		return (&AdapterRequestUpdateOne{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpDelete, OpDeleteOne:
		return (&AdapterRequestDelete{config: c.config, hooks: c.Hooks(), mutation: m}).Exec(ctx)
	default:
		return nil, fmt.Errorf("ent: unknown AdapterRequest mutation op: %q", m.Op())
	}
}

// AdapterUsageRecordClient is a client for the AdapterUsageRecord schema.
type AdapterUsageRecordClient struct {
	config
}

// NewAdapterUsageRecordClient returns a client for the AdapterUsageRecord from the given config.
func NewAdapterUsageRecordClient(c config) *AdapterUsageRecordClient {
	return &AdapterUsageRecordClient{config: c}
}

// Use adds a list of mutation hooks to the hooks stack.
// A call to `Use(f, g, h)` equals to `adapterusagerecord.Hooks(f(g(h())))`.
func (c *AdapterUsageRecordClient) Use(hooks ...Hook) {
	c.hooks.AdapterUsageRecord = append(c.hooks.AdapterUsageRecord, hooks...)
}

// Intercept adds a list of query interceptors to the interceptors stack.
// A call to `Intercept(f, g, h)` equals to `adapterusagerecord.Intercept(f(g(h())))`.
func (c *AdapterUsageRecordClient) Intercept(interceptors ...Interceptor) {
	c.inters.AdapterUsageRecord = append(c.inters.AdapterUsageRecord, interceptors...)
}

// Create returns a builder for creating a AdapterUsageRecord entity.
func (c *AdapterUsageRecordClient) Create() *AdapterUsageRecordCreate {
	mutation := newAdapterUsageRecordMutation(c.config, OpCreate)
	return &AdapterUsageRecordCreate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// CreateBulk returns a builder for creating a bulk of AdapterUsageRecord entities.
func (c *AdapterUsageRecordClient) CreateBulk(builders ...*AdapterUsageRecordCreate) *AdapterUsageRecordCreateBulk {
	return &AdapterUsageRecordCreateBulk{config: c.config, builders: builders}
}

// MapCreateBulk creates a bulk creation builder from the given slice. For each item in the slice, the function creates
// a builder and applies setFunc on it.
func (c *AdapterUsageRecordClient) MapCreateBulk(slice any, setFunc func(*AdapterUsageRecordCreate, int)) *AdapterUsageRecordCreateBulk {
	rv := reflect.ValueOf(slice)
	if rv.Kind() != reflect.Slice {
		return &AdapterUsageRecordCreateBulk{err: fmt.Errorf("calling to AdapterUsageRecordClient.MapCreateBulk with wrong type %T, need slice", slice)}
	}
	builders := make([]*AdapterUsageRecordCreate, rv.Len())
	for i := 0; i < rv.Len(); i++ {
		builders[i] = c.Create()
		setFunc(builders[i], i)
	}
...
// Update returns an update builder for AdapterUsageRecord.
func (c *AdapterUsageRecordClient) Update() *AdapterUsageRecordUpdate {
	mutation := newAdapterUsageRecordMutation(c.config, OpUpdate)
	return &AdapterUsageRecordUpdate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOne returns an update builder for the given entity.
func (c *AdapterUsageRecordClient) UpdateOne(_m *AdapterUsageRecord) *AdapterUsageRecordUpdateOne {
	mutation := newAdapterUsageRecordMutation(c.config, OpUpdateOne, withAdapterUsageRecord(_m))
	return &AdapterUsageRecordUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOneID returns an update builder for the given id.
func (c *AdapterUsageRecordClient) UpdateOneID(id int64) *AdapterUsageRecordUpdateOne {
	mutation := newAdapterUsageRecordMutation(c.config, OpUpdateOne, withAdapterUsageRecordID(id))
	return &AdapterUsageRecordUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// Delete returns a delete builder for AdapterUsageRecord.
func (c *AdapterUsageRecordClient) Delete() *AdapterUsageRecordDelete {
	mutation := newAdapterUsageRecordMutation(c.config, OpDelete)
	return &AdapterUsageRecordDelete{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// DeleteOne returns a builder for deleting the given entity.
func (c *AdapterUsageRecordClient) DeleteOne(_m *AdapterUsageRecord) *AdapterUsageRecordDeleteOne {
	return c.DeleteOneID(_m.ID)
}

// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *AdapterUsageRecordClient) DeleteOneID(id int64) *AdapterUsageRecordDeleteOne {
	builder := c.Delete().Where(adapterusagerecord.ID(id))
	builder.mutation.id = &id
	builder.mutation.op = OpDeleteOne
	return &AdapterUsageRecordDeleteOne{builder}
}

// Query returns a query builder for AdapterUsageRecord.
func (c *AdapterUsageRecordClient) Query() *AdapterUsageRecordQuery {
	return &AdapterUsageRecordQuery{
		config: c.config,
		ctx:    &QueryContext{Type: TypeAdapterUsageRecord},
		inters: c.Interceptors(),
	}
}

// Get returns a AdapterUsageRecord entity by its id.
func (c *AdapterUsageRecordClient) Get(ctx context.Context, id int64) (*AdapterUsageRecord, error) {
	return c.Query().Where(adapterusagerecord.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *AdapterUsageRecordClient) GetX(ctx context.Context, id int64) *AdapterUsageRecord {
	obj, err := c.Get(ctx, id)
	if err != nil {
		panic(err)
	}
...
// QueryUser queries the user edge of a AdapterUsageRecord.
func (c *AdapterUsageRecordClient) QueryUser(_m *AdapterUsageRecord) *UserQuery {
	query := (&UserClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(adapterusagerecord.Table, adapterusagerecord.FieldID, id),
			sqlgraph.To(user.Table, user.FieldID),
...
// QueryAPIKey queries the api_key edge of a AdapterUsageRecord.
func (c *AdapterUsageRecordClient) QueryAPIKey(_m *AdapterUsageRecord) *APIKeyQuery {
	query := (&APIKeyClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
...
// QueryGroup queries the group edge of a AdapterUsageRecord.
func (c *AdapterUsageRecordClient) QueryGroup(_m *AdapterUsageRecord) *GroupQuery {
	query := (&GroupClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
...
// QueryAdapterProvider queries the adapter_provider edge of a AdapterUsageRecord.
func (c *AdapterUsageRecordClient) QueryAdapterProvider(_m *AdapterUsageRecord) *AdapterProviderQuery {
	query := (&AdapterProviderClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
...
// QueryRoutePolicy queries the route_policy edge of a AdapterUsageRecord.
func (c *AdapterUsageRecordClient) QueryRoutePolicy(_m *AdapterUsageRecord) *RoutePolicyQuery {
	query := (&RoutePolicyClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
...
// Hooks returns the client hooks.
func (c *AdapterUsageRecordClient) Hooks() []Hook {
	return c.hooks.AdapterUsageRecord
}

// Interceptors returns the client interceptors.
func (c *AdapterUsageRecordClient) Interceptors() []Interceptor {
	return c.inters.AdapterUsageRecord
}

func (c *AdapterUsageRecordClient) mutate(ctx context.Context, m *AdapterUsageRecordMutation) (Value, error) {
	switch m.Op() {
	case OpCreate:
		return (&AdapterUsageRecordCreate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdate:
		return (&AdapterUsageRecordUpdate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdateOne:
		return (&AdapterUsageRecordUpdateOne{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpDelete, OpDeleteOne:
		return (&AdapterUsageRecordDelete{config: c.config, hooks: c.Hooks(), mutation: m}).Exec(ctx)
	default:
		return nil, fmt.Errorf("ent: unknown AdapterUsageRecord mutation op: %q", m.Op())
	}
}
...
// QueryAdapterRequests queries the adapter_requests edge of a Group.
func (c *GroupClient) QueryAdapterRequests(_m *Group) *AdapterRequestQuery {
	query := (&AdapterRequestClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(group.Table, group.FieldID, id),
			sqlgraph.To(adapterrequest.Table, adapterrequest.FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, group.AdapterRequestsTable, group.AdapterRequestsColumn),
		)
		fromV = sqlgraph.Neighbors(_m.driver.Dialect(), step)
		return fromV, nil
	}
...
// QueryAdapterUsageRecords queries the adapter_usage_records edge of a Group.
func (c *GroupClient) QueryAdapterUsageRecords(_m *Group) *AdapterUsageRecordQuery {
	query := (&AdapterUsageRecordClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(group.Table, group.FieldID, id),
			sqlgraph.To(adapterusagerecord.Table, adapterusagerecord.FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, group.AdapterUsageRecordsTable, group.AdapterUsageRecordsColumn),
		)
		fromV = sqlgraph.Neighbors(_m.driver.Dialect(), step)
		return fromV, nil
	}
...
// UpdateOneID returns an update builder for the given id.
func (c *RoutePolicyClient) UpdateOneID(id int64) *RoutePolicyUpdateOne {
...
// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *RoutePolicyClient) DeleteOneID(id int64) *RoutePolicyDeleteOne {
...
func (c *RoutePolicyClient) Get(ctx context.Context, id int64) (*RoutePolicy, error) {
	return c.Query().Where(routepolicy.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *RoutePolicyClient) GetX(ctx context.Context, id int64) *RoutePolicy {
	obj, err := c.Get(ctx, id)
	if err != nil {
		panic(err)
	}
...
// QueryAdapterProvider queries the adapter_provider edge of a RoutePolicy.
func (c *RoutePolicyClient) QueryAdapterProvider(_m *RoutePolicy) *AdapterProviderQuery {
	query := (&AdapterProviderClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
...
// QueryAdapterUsageRecords queries the adapter_usage_records edge of a RoutePolicy.
func (c *RoutePolicyClient) QueryAdapterUsageRecords(_m *RoutePolicy) *AdapterUsageRecordQuery {
	query := (&AdapterUsageRecordClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
...
// QueryAdapterRequests queries the adapter_requests edge of a User.
func (c *UserClient) QueryAdapterRequests(_m *User) *AdapterRequestQuery {
	query := (&AdapterRequestClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(user.Table, user.FieldID, id),
			sqlgraph.To(adapterrequest.Table, adapterrequest.FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, user.AdapterRequestsTable, user.AdapterRequestsColumn),
		)
		fromV = sqlgraph.Neighbors(_m.driver.Dialect(), step)
		return fromV, nil
	}
...
// QueryAdapterUsageRecords queries the adapter_usage_records edge of a User.
func (c *UserClient) QueryAdapterUsageRecords(_m *User) *AdapterUsageRecordQuery {
	query := (&AdapterUsageRecordClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(user.Table, user.FieldID, id),
			sqlgraph.To(adapterusagerecord.Table, adapterusagerecord.FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, user.AdapterUsageRecordsTable, user.AdapterUsageRecordsColumn),
		)
		fromV = sqlgraph.Neighbors(_m.driver.Dialect(), step)
		return fromV, nil
	}
...
type (
	hooks struct {
		APIKey, Account, AccountGroup, AdapterProvider, AdapterRequest,
		AdapterUsageRecord, Announcement, AnnouncementRead, AuthIdentity,
		AuthIdentityChannel, ChannelMonitor, ChannelMonitorDailyRollup,
		ChannelMonitorHistory, ChannelMonitorRequestTemplate, ErrorPassthroughRule,
		Group, IdempotencyRecord, IdentityAdoptionDecision, PaymentAuditLog,
		PaymentOrder, PaymentProviderInstance, PendingAuthSession, PromoCode,
		PromoCodeUsage, Proxy, RedeemCode, RoutePolicy, SecuritySecret, Setting,
