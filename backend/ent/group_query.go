...
func (_q *GroupQuery) QueryAdapterRequests() *AdapterRequestQuery {
...
func (_q *GroupQuery) QueryAdapterUsageRecords() *AdapterUsageRecordQuery {
...
func (_q *GroupQuery) WithAdapterRequests(opts ...func(*AdapterRequestQuery)) *GroupQuery {
	query := (&AdapterRequestClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *GroupQuery) WithAdapterUsageRecords(opts ...func(*AdapterUsageRecordQuery)) *GroupQuery {
	query := (&AdapterUsageRecordClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := _q.withAdapterRequests; query != nil {
		if err := _q.loadAdapterRequests(ctx, query, nodes,
			func(n *Group) { n.Edges.AdapterRequests = []*AdapterRequest{} },
			func(n *Group, e *AdapterRequest) { n.Edges.AdapterRequests = append(n.Edges.AdapterRequests, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withAdapterUsageRecords; query != nil {
		if err := _q.loadAdapterUsageRecords(ctx, query, nodes,
			func(n *Group) { n.Edges.AdapterUsageRecords = []*AdapterUsageRecord{} },
			func(n *Group, e *AdapterUsageRecord) {
...
func (_q *GroupQuery) loadAdapterRequests(ctx context.Context, query *AdapterRequestQuery, nodes []*Group, init func(*Group), assign func(*Group, *AdapterRequest)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Group)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(adapterrequest.FieldGroupID)
	}
	query.Where(predicate.AdapterRequest(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(group.AdapterRequestsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *GroupQuery) loadAdapterUsageRecords(ctx context.Context, query *AdapterUsageRecordQuery, nodes []*Group, init func(*Group), assign func(*Group, *AdapterUsageRecord)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Group)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(adapterusagerecord.FieldGroupID)
	}
	query.Where(predicate.AdapterUsageRecord(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(group.AdapterUsageRecordsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
