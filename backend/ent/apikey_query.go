...
func (_q *APIKeyQuery) QueryAdapterRequests() *AdapterRequestQuery {
...
func (_q *APIKeyQuery) QueryAdapterUsageRecords() *AdapterUsageRecordQuery {
...
func (_q *APIKeyQuery) WithAdapterRequests(opts ...func(*AdapterRequestQuery)) *APIKeyQuery {
	query := (&AdapterRequestClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *APIKeyQuery) WithAdapterUsageRecords(opts ...func(*AdapterUsageRecordQuery)) *APIKeyQuery {
	query := (&AdapterUsageRecordClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *APIKeyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*APIKey, error) {
	...
	if query := _q.withAdapterRequests; query != nil {
		if err := _q.loadAdapterRequests(ctx, query, nodes,
			func(n *APIKey) { n.Edges.AdapterRequests = []*AdapterRequest{} },
			func(n *APIKey, e *AdapterRequest) { n.Edges.AdapterRequests = append(n.Edges.AdapterRequests, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withAdapterUsageRecords; query != nil {
		if err := _q.loadAdapterUsageRecords(ctx, query, nodes,
			func(n *APIKey) { n.Edges.AdapterUsageRecords = []*AdapterUsageRecord{} },
			func(n *APIKey, e *AdapterUsageRecord) {
...
func (_q *APIKeyQuery) loadAdapterRequests(ctx context.Context, query *AdapterRequestQuery, nodes []*APIKey, init func(*APIKey), assign func(*APIKey, *AdapterRequest)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*APIKey)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(adapterrequest.FieldAPIKeyID)
	}
	query.Where(predicate.AdapterRequest(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(apikey.AdapterRequestsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *APIKeyQuery) loadAdapterUsageRecords(ctx context.Context, query *AdapterUsageRecordQuery, nodes []*APIKey, init func(*APIKey), assign func(*APIKey, *AdapterUsageRecord)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*APIKey)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(adapterusagerecord.FieldAPIKeyID)
	}
	query.Where(predicate.AdapterUsageRecord(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(apikey.AdapterUsageRecordsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
