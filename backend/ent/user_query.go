...
func (_q *UserQuery) QueryAdapterRequests() *AdapterRequestQuery {
...
func (_q *UserQuery) QueryAdapterUsageRecords() *AdapterUsageRecordQuery {
...
func (_q *UserQuery) WithAdapterRequests(opts ...func(*AdapterRequestQuery)) *UserQuery {
	query := (&AdapterRequestClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) WithAdapterUsageRecords(opts ...func(*AdapterUsageRecordQuery)) *UserQuery {
	query := (&AdapterUsageRecordClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withAdapterRequests; query != nil {
		if err := _q.loadAdapterRequests(ctx, query, nodes,
			func(n *User) { n.Edges.AdapterRequests = []*AdapterRequest{} },
			func(n *User, e *AdapterRequest) { n.Edges.AdapterRequests = append(n.Edges.AdapterRequests, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withAdapterUsageRecords; query != nil {
		if err := _q.loadAdapterUsageRecords(ctx, query, nodes,
			func(n *User) { n.Edges.AdapterUsageRecords = []*AdapterUsageRecord{} },
			func(n *User, e *AdapterUsageRecord) {
...
func (_q *UserQuery) loadAdapterRequests(ctx context.Context, query *AdapterRequestQuery, nodes []*User, init func(*User), assign func(*User, *AdapterRequest)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(adapterrequest.FieldUserID)
	}
	query.Where(predicate.AdapterRequest(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.AdapterRequestsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *UserQuery) loadAdapterUsageRecords(ctx context.Context, query *AdapterUsageRecordQuery, nodes []*User, init func(*User), assign func(*User, *AdapterUsageRecord)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(adapterusagerecord.FieldUserID)
	}
	query.Where(predicate.AdapterUsageRecord(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.AdapterUsageRecordsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
