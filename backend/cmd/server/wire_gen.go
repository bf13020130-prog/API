...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	adapterProviderRepository := repository.NewAdapterProviderRepository(client)
	adapterProviderService := service.NewAdapterProviderService(adapterProviderRepository)
	routePolicyRepository := repository.NewRoutePolicyRepository(client)
	routePolicyService := service.NewRoutePolicyService(routePolicyRepository, adapterProviderService)
	adapterRequestRepository := repository.NewAdapterRequestRepository(client)
	adapterRequestService := service.NewAdapterRequestService(adapterRequestRepository)
	adapterUsageRepository := repository.NewAdapterUsageRepository(client)
	adapterUsageService := service.NewAdapterUsageService(adapterUsageRepository)
	systemHandler := handler.ProvideSystemHandler(updateService, systemOperationLockService, adapterProviderService, routePolicyService, adapterRequestService, adapterUsageService)
	...
	adapterclientClient := service.ProvideAdapterClient(adapterProviderService)
	adapterEnforcementService := service.ProvideAdapterEnforcementService(configConfig, routePolicyService, adapterProviderService, adapterRequestService, adapterUsageService, adapterclientClient, usageBillingRepository)
	engine := server.ProvideRouter(configConfig, handlers, jwtAuthMiddleware, adminAuthMiddleware, apiKeyAuthMiddleware, apiKeyService, subscriptionService, opsService, settingService, adapterProviderService, adapterEnforcementService, redisClient)
