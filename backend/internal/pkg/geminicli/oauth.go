...
//
// If ClientID/ClientSecret is not provided, this falls back to an operator-provided
// Gemini CLI OAuth client from environment variables.
//
...
func EffectiveOAuthConfig(cfg OAuthConfig, oauthType string) (OAuthConfig, error) {
	...
	// Fall back to operator-provided Gemini CLI OAuth client when not configured.
...
func IsGeminiCLIOAuthClient(clientID string) bool {
	...
	return clientID != "" && clientID == strings.TrimSpace(os.Getenv(GeminiCLIOAuthClientIDEnv))
}
