...
type GatewayConfig struct {
	...
	// AdapterProviders: new-api 仅作为长尾供应商适配器边界；默认空列表，不接管核心供应商。
	AdapterProviders []GatewayAdapterProviderConfig `mapstructure:"adapter_providers"`
	// AdapterEnforcement: route_policies 命中长尾供应商时是否短路到 adapter；默认关闭。
	AdapterEnforcement GatewayAdapterEnforcementConfig `mapstructure:"adapter_enforcement"`
...
type GatewayAdapterProviderConfig struct {
...
type GatewayAdapterEnforcementConfig struct {
	Enabled            bool                                   `mapstructure:"enabled"`
	DiagnosticSampling GatewayAdapterDiagnosticSamplingConfig `mapstructure:"diagnostic_sampling"`
}

type GatewayAdapterDiagnosticSamplingConfig struct {
	Enabled         bool     `mapstructure:"enabled"`
	SampleAll       bool     `mapstructure:"sample_all"`
	Providers       []string `mapstructure:"providers"`
	RequestIDs      []string `mapstructure:"request_ids"`
...
func setDefaults() {
	...
	viper.SetDefault("gateway.adapter_enforcement.enabled", false)
	viper.SetDefault("gateway.adapter_enforcement.diagnostic_sampling.enabled", false)
	viper.SetDefault("gateway.adapter_enforcement.diagnostic_sampling.sample_all", false)
	viper.SetDefault("gateway.adapter_enforcement.diagnostic_sampling.providers", []string{})
	viper.SetDefault("gateway.adapter_enforcement.diagnostic_sampling.request_ids", []string{})
	viper.SetDefault("gateway.adapter_enforcement.diagnostic_sampling.max_payload_bytes", 4096)
	viper.SetDefault("gateway.adapter_enforcement.diagnostic_sampling.max_string_bytes", 512)
	viper.SetDefault("gateway.adapter_enforcement.diagnostic_sampling.max_events", 3)
