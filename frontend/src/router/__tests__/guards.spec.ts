...
describe('路由守卫逻辑', () => {
  ...
  describe('简易模式受限路由', () => {
    ...
    it('管理员简易模式访问 /admin/adapters 重定向到 /admin/dashboard', () => {
      const authState: MockAuthState = {
        isAuthenticated: true,
        isAdmin: true,
        isSimpleMode: true,
      ...
      const redirect = simulateGuard('/admin/adapters', { requiresAdmin: true }, authState)
      expect(redirect).toBe('/admin/dashboard')
    })

    it('管理员简易模式访问渠道管理适配器兼容入口也重定向', () => {
      const authState: MockAuthState = {
        isAuthenticated: true,
        isAdmin: true,
        isSimpleMode: true,
      ...
      const redirect = simulateGuard('/admin/channels/adapters', { requiresAdmin: true }, authState)
      expect(redirect).toBe('/admin/dashboard')
    })
